 ...
 *
*/
#include "conveyor_gazebo/plugins/ROSConveyorBeltPlugin.hh"
...
bool ROSConveyorBeltPlugin::OnControlCommand(ros::ServiceEvent<
  conveyor_gazebo::ConveyorBeltControl::Request, conveyor_gazebo::ConveyorBeltControl::Response> & event)
{
  const conveyor_gazebo::ConveyorBeltControl::Request& req = event.getRequest();
  conveyor_gazebo::ConveyorBeltControl::Response& res = event.getResponse();
  ...
  // During the ARIAC competition, this environment variable will be set.
